...
impl Codegen {
    ...
    pub fn generate_comment_header(&mut self) {
        self.headers += "/**\n";
        self.headers += " * This file was generated by the Zen compiler. Do not edit this\n";
        self.headers += " * file directly, as changes may create unexpected behavior in\n";
        self.headers += " * the generated code.\n";
        self.headers += " */\n";
    }

    /// Generate the necessary includes for the generated C code.
    pub fn generate_includes(&mut self) {
        self.headers += "#include <stdio.h>\n";
        self.headers += "#include <stdbool.h>\n";
        self.headers += "#include <stddef.h>\n";
    }
    ...
    pub fn generate_fn_header(&mut self, func: &ast::Function, prototype: bool) {
        ...
        code += &ret.ctype; // FIXME: Handle arrays in return types
        code += " ";
        code += func.prototype.ident.name;
        code += "(";
    ...
    pub fn generate_fn(&mut self, func: &ast::Function) {
        ...
        self.scoped(|codegen| {
            ...
            if func.body.is_empty() {
                codegen.functions += &codegen.indent_codegen();
                codegen.functions += "// empty function body\n";
            } else {
                for stmt in &func.body {
                    let code = &codegen.generate_stmt(stmt);
                    codegen.functions += &codegen.indent_codegen();
                    codegen.functions += code;
                    codegen.functions += "\n";
                }
            }
            codegen.functions += "}\n";
            ...
            ast::ExprKind::IntrinsicCall(ident, args) => match ident.name {
                "println" | "print" => {
                    ...
                    let s = args[0]
                        .kind
                        .as_string_literal()
                        .expect("Expected string literal for print/println format string");
...
pub struct TypeInfo {
    ...
    /// The postfix to be added to the type when declaring variables of that type.
    pub postfix: String,
}

impl TypeInfo {
    /// Create a new TypeInfo object for a simple type that does not require
    /// any postfix (e.g. bool, int, void, etc.).
    #[must_use]
    pub fn simple(ctype: String) -> Self {
        Self {
            ctype,
            postfix: String::new(),
        }
    ...
    /// Create a new TypeInfo object for an array type.
    #[must_use]
    pub fn array(ctype: String, size: u64) -> Self {
        Self {
            ctype,
            postfix: format!("[{}]", size),
        }
